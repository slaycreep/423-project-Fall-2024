...
def immunity_timer():
    ...
    if immunity_active:
        ...
        if immunity_time <= 0:
            ...
            print("Immunity deactivated")  # End immunity
...
def check_planes():
    global lstCreatedPlane, planeLst
    if len(planeLst) == 0 and random.randint(1, 100) == 10:
        create_plane()
        lstCreatedPlane = time.time()

def check_airplane_balloon_collision():
    global planeLst, gameover, immunity_active
    if immunity_active:
        return False
...
def mouseListener(button,state,x,y):
    global canyon_top,c,b,cl,sky,clouds,gameover,fuel_level,game_state,fuel_cans,immunity_cans,is_immune,immunity_time,immunity_active,immunity_duration,pause_time,pause_start_time, start_time, birds, slow, planeLst, planeSpeed, lstCreatedPlane
    actualx = x
    actualy = 500 - y
...
def showScreen():
    ...
    if not gameover:
        ...
        # immunity related display
        immunity_powerup()
        draw_immunity_cans()
        # draw_immunity_timer()

        if immunity_can_collision():
